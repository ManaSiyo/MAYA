...
for(const stage of ['passed','in_process','in_progress','booked','canceled','new']){await vm.runInContext(`updateLead('m_one',{stage:'${stage}'})`,context);assert.equal(record.items[0].stage,stage);checks++;}
assert.equal(await vm.runInContext("updateLead('m_one',{stage:'invalid'})",context),null);assert.equal(writes,8);checks++;
...
const crm=vm.createContext({document:{addEventListener:()=>{}}});vm.runInContext(admin.slice(admin.indexOf('const LEAD_STAGES='),admin.indexOf('const LEAD_COLS_DEFAULT')),crm);
